...
S123.ReplaceExternalLinksInFreeSubDomain = function() {
    ...
    if (!websiteID || !websiteDomain || !isWhiteLabelSubDomainBOO) return;
    if (isWhiteLabelSubDomainBOO == '0') return;
    ...
    if ($s_whitelabel['domain'].length !== 0) allowedDomainsARR.push($s_whitelabel['domain']);
    if ($GLOBALS['cdn-user-files'].length !== 0) allowedDomainsARR.push($GLOBALS['cdn-user-files']);
    if ($GLOBALS['cdn-user-videos-files'].length !== 0) allowedDomainsARR.push($GLOBALS['cdn-user-videos-files']);
    if ($GLOBALS['cdn-interface-files'].length !== 0) allowedDomainsARR.push($GLOBALS['cdn-interface-files']);
    if ($GLOBALS['cdn-system-files'].length !== 0) allowedDomainsARR.push($GLOBALS['cdn-system-files']);
    if ($GLOBALS['cdn-images-files'].length !== 0) allowedDomainsARR.push($GLOBALS['cdn-images-files']);
    $(document).on('s123.page.ready.ReplaceExternalLinksInFreeSubDomain', function(event) {
        $('a').each(function() {
            let $link = $(this);
            let hrefTXT = $link.attr('href');
            if ($link.attr('rel') === 'nofollow') return true;
            if (!hrefTXT) return true;
            if (hrefTXT.indexOf('external_redirect.php') !== -1) return true;
            if (hrefTXT.indexOf('http://') === 0 || hrefTXT.indexOf('https://') === 0) {
                let isAllowedDomainBOO = false;
                $.each(allowedDomainsARR, function(index, allowedDomainTXT) {
                    if (hrefTXT.indexOf(allowedDomainTXT) !== -1) {
                        isAllowedDomainBOO = true;
                        return false;
                    }
                });
                if (!isAllowedDomainBOO) {
                    var newUrl = window.location.protocol + '//' + websiteDomain +
                        '/versions/2/include/external_redirect.php?websiteID=' + encodeURIComponent(websiteID) + '&url=' + encodeURIComponent(hrefTXT);
...
function WizardNotificationUpdate() {
    if (IsWizard()) topWindow.WebsiteNotification.update();
}
...
var site123FixText = function() {
    ...
    _.itemFixText = function($textEle, $parentBox, compressor) {
        ...
        if ($textEle.length > 1) {
            ...
            if (fitText_max.isfitText) $textEle.css('font-size', fitText_max.maxFontSize + 'px');
        } else {
            const fitText = _.fixText($textEle, $parentBox, compressor);
            if (fitText.isFitText) $textEle.css('font-size', fitText.minFontSize + 'px');
        }
